...
exports.publishTheArticle = (req, res, callback) => {
  ...
  if (req.user._moderator) {
    ...
    if (data.article_id) {
      ...
      // when the article does not exist ,save to published articles collection directly
    ...
    // save to pending articles collection directly
    if (data.article_id) {
    ...
    Article.newPendingArticle(data, function (err, article) {
      if (err) {
        ...
        return res.json({
          success: true,
          error_info: err,
          article_id: article._id /*,auth_token: req.header('auth-token')*/,
        });
      }
    });
  }
...
exports.getTheArticle = (req, res, callback) => {
  Article.getArticleById(req.params.id, function (err, article) {
    if (err || article === null) {
      return res.json({
        success: false,
        error_info: 'the article is not exist',
        auth_token: req.header('auth-token'),
      });
    } else {
...
exports.getTheArticle2 = (req, res, callback) => {
  Article.getArticleById(req.params.id, function (err, article) {
    if (err || article === null) {
      return res.json({
        success: false,
        error_info: 'Article does not exist!',
        auth_token: req.header('auth-token'),
      });
    }
    ...
    if (article.current_article) {
      User.getUserById2(article.author_list[0], function (err, user) {
        if (err) {
          ...
          return res.json({
            success: false,
            returnValuesForArticle: returnValuesForArticle,
            auth_token: req.header('auth-token'),
          });
        }
      ...
      return res.json({
        success: false,
        error_info: 'This article have not been approved by the moderator yet',
        returnValuesForArticle: returnValuesForArticle,
        auth_token: req.header('auth-token'),
      });
  });
...
exports.deleteTheArticle = (req, res, callback) => {
  if (!req.user._moderator) {
    return res.json({
      success: false,
      reason: 'student have no authority to delete',
      auth_token: req.header('auth-token'),
    });
  } else {
      ...
      Article.getArticleById(req.params.id, function (err, article) {
        if (err || article === null) {
          ...
          if (article.is_pending === true) {
            return res.json({
              success: false,
              err_info: 'already a pending article, cannot edit',
              auth_token: req.header('auth-token'),
            });
          }
          returnValuesForArticle.title = article.title;
  ...
  Article.getPendingArticleById(req.params.id, function (err, article) {
    if (err || article === null) {
      return res.json({
        success: false,
        error_info:
          'this pending article does not exist,the article has been processed by moderator just now',
        auth_token: req.header('auth-token'),
      });
    } else {
      if (
        article.editor_id != null &&
        article.editor_id.toString() === req.user._id
      ) {
        returnValuesForArticle.title = article.title;
        returnValuesForArticle.content = article.content;
        returnValuesForArticle.id = req.params.id;
...
exports.getAllNamesOfTheArticles = (req, res, callback) => {
  Article.getAllArticles(function (error, articles) {
    if (error) {
      return res.json({
        success: false,
        error_info: 'cannot get articles name',
        auth_token: req.header('auth-token'),
      });
    }
    let articleInfo = [];
...
exports.approvePendingArticle = (req, res, callback) => {
  if (req.user._moderator) {
    ...
    Article.getPendingArticleById(
      pending_id,
      function (error, pending_article) {
        if (error) {
          return res.json({
            success: false,
            error_info: error,
          ...
          if (pending_article.editor_id != null) {
            update.author_id = mongoose.Types.ObjectId(
              pending_article.editor_id
            );
          }
          update.is_pending = false;
          if (pending_article && pending_article.published_article) {
            Article.updateArticle(
              pending_article.published_article,
              update,
              function (err, article) {
                if (err) {
                  return res.json({
                  ...
                  Article.deletePendingArticleById(
            ...
            return res.json({
              success: false,
              error_info: 'pending article does not exist',
              auth_token: req.header('auth-token'),
            });
          }
        }
    ...
    return res.json({
      success: false,
      error_info: 'Only moderators can reject articles',
      auth_token: req.header('auth-token'),
    });
  }
};
  ...
  if (req.user._moderator) {
    Article.getPendingArticleById(req.params.id, function (err, article) {
      if (err || article === null) {
        return res.json({
          success: false,
          error_info: 'the article has been processed by other moderator',
          auth_token: req.header('auth-token'),
        });
      }
    ...
    return res.json({
      success: false,
      error_info: 'student have no author to get details of pending article',
      auth_token: req.header('auth-token'),
    });
  }
};
...
exports.getPendingArticlesByUserId = (req, res, callback) => {
  Article.findPendingArticleByUserId(req.user._id, function (error, articles) {
    if (error) {
      return res.json({
        success: false,
        error_info: 'cannot get all pending articles for this user',
        auth_token: req.header('auth-token'),
      });
    }
    let articleInfo = [];
...
exports.getPublishedArticlesByUserId = (req, res, callback) => {
  Article.findPublishedArticleByUserId(
    req.user._id,
    function (error, articles) {
      if (error) {
        return res.json({
...
exports.likeArticle = async (req, res, callback) => {
  ...
  Article.likeArticle(article_id, user_id, function (error, article) {
    if (error) {
      return res
        .status(400)
        .json({success: false, error, auth_token: req.header('auth-token')});
    }
    return res
      .status(200)
      .json({
...
exports.bookmarkArticle = async (req, res, callback) => {
  let article_id = req.params.id;
  let user_id = req.user._id;
  Article.bookmarkArticle(article_id, user_id, function (error, article) {
    if (error) {
      return res
        .status(400)
        .json({success: false, error, auth_token: req.header('auth-token')});
...
exports.unlikeArticle = async (req, res, callback) => {
  let article_id = req.params.id;
  let user_id = req.user._id;
  Article.unlikeArticle(article_id, user_id, function (error, article) {
    if (error) {
      return res
        .status(400)
        .json({success: false, error, auth_token: req.header('auth-token')});
...
exports.unbookmarkArticle = async (req, res, callback) => {
  let article_id = req.params.id;
  let user_id = req.user._id;
  Article.unbookmarkArticle(article_id, user_id, function (error, article) {
    if (error) {
      return res
        .status(400)
        .json({success: false, error, auth_token: req.header('auth-token')});
